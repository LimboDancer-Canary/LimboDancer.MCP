# 4. **MCP tool surface for history/memory/graph (first useful tools)**
   ...
   * Why: Makes the server actually useful while we wire KG/vector; aligns with the design map and ontology-first approach. &#x20;
   ...
   Awesome — here are the **four MCP tools** as PR-ready files.
They follow the same **manual handler** model you used earlier (MCP C# SDK), and each tool’s `input_schema` embeds the **JSON-LD `@context`** from your ontology so fields are ontology-bound.
